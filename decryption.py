...
def plain_text_permutation2_decrypt(keys, binary_string):
    L = binary_string[:32]
    R = binary_string[32:]
...
def main():
    ...
    while default not in ('y', 'Y', 'n', 'N'):
        ...
        if default in ('y', 'Y'):
            ...
            K = input('Please enter a 16 digit hexadecimal number for the key variable: ')
        else:
    ...
    plain_text_p2 = plain_text_permutation2_decrypt(p2, IP)
    hex_string = hex(int(plain_text_p2, 2))[2:].upper().zfill(16)
    print(f'\nDecrypted Text: {hex_string}\n')
